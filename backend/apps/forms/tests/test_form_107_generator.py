...
class TestComputeAnnualIncome:
    ...
    def test_annual_from_uniform_income(self):
        ...
        assert _compute_annual_income_from_monthly([3000, 3000, 3000, 3000, 3000, 3000]) == Decimal(
            "36000.00"
        )

    def test_annual_from_varying_income(self):
        # (2000 + 3000 + 2500 + 3500 + 2000 + 3000) / 6 = 2666.67 * 12 = 32000.04
        result = _compute_annual_income_from_monthly([2000, 3000, 2500, 3500, 2000, 3000])
    ...
    def test_annual_from_zeros(self):
        assert _compute_annual_income_from_monthly([0, 0, 0, 0, 0, 0]) == ZERO

    def test_monthly_average(self):
        assert _compute_total_monthly_income([3000, 3000, 3000, 3000, 3000, 3000]) == Decimal(
            "3000.00"
        )
...
class TestForm107Generator:
    ...
    def test_all_question_texts_present(self):
        ...
        for q in result["questions"]:
            assert q["question_text"] != "", f"Q{q['question_number']} has empty question_text"
