...
class BalanceSheetExtraction(BaseExtractionSchema):
    ...
    as_of_date: date = Field(description="Balance sheet date (YYYY-MM-DD)")

    # Assets
    cash: Decimal | None = Field(default=None, ge=0, description="Cash and cash equivalents")
    accounts_receivable: Decimal | None = Field(
        default=None, ge=0, description="Accounts receivable"
    )
    inventory: Decimal | None = Field(default=None, ge=0, description="Inventory value")
    equipment: Decimal | None = Field(default=None, ge=0, description="Equipment and fixed assets")
    total_assets: Decimal = Field(gt=0, description="Total assets")

    # Liabilities
    accounts_payable: Decimal | None = Field(default=None, ge=0, description="Accounts payable")
    loans_payable: Decimal | None = Field(default=None, ge=0, description="Loans and notes payable")
    total_liabilities: Decimal = Field(ge=0, description="Total liabilities")

    # Equity
    owners_equity: Decimal = Field(description="Owner's equity (can be negative)")
...
class ProfitLossExtraction(BaseExtractionSchema):
    ...
    period_start: date = Field(description="P&L period start date")
    period_end: date = Field(description="P&L period end date")
    total_revenue: Decimal = Field(ge=0, description="Total revenue for period")
    total_expenses: Decimal = Field(ge=0, description="Total expenses for period")
    net_income: Decimal = Field(description="Net income (can be negative for loss)")
    ...
    def validate_net_income_calculation(cls, v, info):
        ...
        if "total_revenue" in info.data and "total_expenses" in info.data:
            ...
            if abs(calculated - v) > abs(v) * Decimal("0.01"):
                raise ValueError(
                    f"Net income calculation error: " f"{v} ≠ {calculated} (revenue - expenses)"
                )
