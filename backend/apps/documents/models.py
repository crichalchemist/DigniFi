...
class UploadedDocument(models.Model):
    ...
    session = models.ForeignKey(
        "intake.IntakeSession", on_delete=models.CASCADE, related_name="uploaded_documents"
    )
    ...
    user_declared_type = models.CharField(
        max_length=50, choices=DocumentType.choices, help_text="Type user selected before upload"
    )
...
class OCRResult(models.Model):
    ...
    document = models.OneToOneField(
        UploadedDocument, on_delete=models.CASCADE, related_name="ocr_result"
    )
    ...
    overall_confidence = models.DecimalField(
        max_digits=5, decimal_places=2, help_text="Overall confidence score (0-100)"
    )

    # User validation
    user_validated = models.BooleanField(default=False)
    validation_changes = models.JSONField(
        default=list, help_text="List of fields user corrected after extraction"
    )

    # Timing & errors
    processed_at = models.DateTimeField(auto_now_add=True)
    processing_duration = models.FloatField(null=True, help_text="Seconds to process")
