    ...
    function URLify(s, num_chars, allowUnicode) {
        ...
        if (allowUnicode) {
            ...
            if (typeof XRegExp !== 'undefined') {
                ...
                // Fallback: use Unicode property escapes if supported, else ASCII-safe
                try {
                    // Test if Unicode property escapes are supported
                    new RegExp('\\p{L}', 'u');
                    s = s.replace(new RegExp('[^-_\\p{L}\\p{N}\\s]', 'gu'), '');
                } catch (e) {
                    // Fallback to ASCII-safe regex (removes non-ASCII)
                    s = s.replace(/[^-_A-Za-z0-9\s]/g, '');
                }
            }
        } else {
